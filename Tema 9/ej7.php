...
if ($consulta->rowCount() != 0) {
    ...
    if (isset($_REQUEST['code'])) {
        $stock[$_REQUEST['code']] = array ("nom" => $_REQUEST['nom'], "precio" => $_REQUEST['precio'], "img" => $_FILES['img']['name'], "desc" => $_REQUEST['desc']);
        $conexion->exec("INSERT INTO stock (code, nombre, precio, img, descripcion) 
        VALUES ('".$_REQUEST['code']."', '".$_REQUEST['nom']."', '".$_REQUEST['precio']."', '".$_FILES['img']['name']."', '".$_REQUEST['desc']."')");
        move_uploaded_file($_FILES['img']['tmp_name'], "img/".$_FILES['img']['name']);
    }
    if (isset($_REQUEST['rmv'])) {
        unlink("img/".$stock[$_REQUEST['prod']]['img']);
...
<body>
    ...
    <table class="lista">
        ...
        ?>
            <tr>
                <td><a href="ej7Detalle.php?prod=<?=$clave?>"><img src="img/<?= $producto['img'] ?>"></a></td>
                <td><?=$producto['nom']?></td>
    ...
    foreach ($stock as $clave => $producto) {
        if ($carro[$clave] > 0) {
            if ($flag) {
                ...
                <table class="carrito">
                    <tr>
                        <th colspan="3"><a href="ej7carrito.php">Carrito</a></th>
                    </tr>
                    <tr>
                        <th>Juego</th>
                        <th>Cantidad</th>
