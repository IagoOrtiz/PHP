...
<style>
    * {
    ...
    input[value="Entrada"], input[value="Venta"]{
...
if (isset($_REQUEST['mod'])) {
    ...
    if (isset($_REQUEST['code']) && $_REQUEST['code'] != "") {
        ...
        if (isset($_REQUEST['+'])) {
            ...
            if ($stock > $_REQUEST['venta']) {
                $stock-=$_REQUEST['venta'];
                $conexion->exec("UPDATE almacen SET stock = $stock WHERE code = '$_REQUEST[ogcode]'");
                $flag = true;
            } else {
...
<table border="1">
    ...
    while ($almacen = $consulta->fetchObject()) {
        if ($i >= $page && $i < $page+10) {
            if (isset($_REQUEST['ogcode']) && $almacen->code == $_REQUEST['ogcode']) {
                $precio = $almacen->precioVen;
            }
    ?>
        <tr>
            <td><?= $almacen->code ?></td>
            ...
            <form action="ej5mod.php" method="post">
                <input type="hidden" name="code" value="<?= $almacen->code ?>">
                <input type="submit" value="Modificar">
            </form></td>
            <td>
            <form action="ej5stAdd.php" method="post">
                <input type="hidden" name="code" value="<?= $almacen->code ?>">
                <input type="submit" value="Entrada">
            </form>
            <form action="ej5stRmv.php" method="post">
                <input type="hidden" name="code" value="<?= $almacen->code ?>">
...
<?php
    if ($flag) {
        echo "Importe: ".(($_REQUEST['venta']*$precio)*1.21)." €.";
    }
?>
